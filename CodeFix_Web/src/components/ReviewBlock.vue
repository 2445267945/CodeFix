...
const level = computed(
  () =>
    props.block.level || (props.block.status === "failed" ? "error" : "info")
);
const icon = computed(
  () =>
    ({ success: "✓", warning: "!", error: "×", info: "i" }[level.value] || "i")
);
...
.is-success .review-icon {
  color: var(--success);
}
.is-warning .review-icon {
  color: var(--warning);
}
.is-error .review-icon {
  color: var(--danger);
}
.review-title {
  font-size: 12px;
