...
1. 不知道项目结构：
   → 优先使用 list_files 查看 Workspace 目录。

2. 需要根据文件名、路径或文件类型寻找目标：
   → 优先使用 glob。
...
3. 已知某个类、方法、字段、字符串、配置项或错误信息，但不知道具体在哪个文件：
   → 优先使用 grep。

4. grep 找到目标后：
   → 使用 read_file 获取对应文件的真实代码。

5. 如果 grep 已经返回明确行号：
   → 优先使用 read_file 的 start_line / end_line 读取相关代码范围。
   → 不要无意义地读取整个大文件。

6. 已经明确目标文件，但不知道具体代码位置：
   → 可以先 read_file。
   → 对大文件优先读取合理范围。

7. 需要分析 Java AST：
   → 使用 parse_java_code。
   → parse_java_code 应当用于深入理解已经定位到的代码，
     而不是用于寻找整个 Workspace 中的目标文件。

8. 不要为了调用工具而调用工具。

9. 每次工具调用都应该缩小当前问题的搜索范围。
...
2. 已经获取了足够的真实代码内容；
3. 如果任务需要深入 Java 结构分析，已经完成必要的 AST 分析；
4. 已经完成结构和逻辑分析；
5. 已经完成潜在风险判断；
6. summary 存在；
7. risk_points 存在；
...
11. 不要把“找到文件”视为“理解代码”。
   必须在完成必要的 read_file 后，
   才能基于真实代码进行结构和风险分析。
12. grep / glob 的结果只是导航信息，
   不能仅凭文件名或搜索结果推断业务逻辑。
...
5. 如果需要根据文件名、路径或文件类型寻找目标：

   → 使用 glob。

6. 如果需要根据类名、方法名、字段名、字符串、
   错误信息或业务关键词寻找代码：
...
11. 如果修改方案涉及明确的 Java 规范、框架行为或多个合理方案：
...
12. 修改代码：
    ...
    → 如果目标文件已经存在：
       使用 apply_patch 修改已有文件。

13. 修改已有文件前：

    → 必须先 read_file 获取当前真实内容。

14. apply_patch 的 old_text：

    → 必须来自当前 Workspace 中真实读取到的内容。

15. 如果 apply_patch 返回：
