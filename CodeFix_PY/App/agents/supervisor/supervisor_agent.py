...
class SupervisorAgent(ToolExecutor):
    def __init__(self, context: AgentContext, run_context: AgentRunContext, base_message=None, parent_agent=None):
        ...
        3. 如果已知文件名、目录、文件扩展名或命名规律：
           → 优先使用 glob。
           → 例如已知 Login*.java、**/*Controller.java、**/*.xml 等。
        
        4. 如果需要根据类名、方法名、字段名、字符串、
           配置项、错误信息或业务关键词定位代码内容：
           → 优先使用 grep。
           → grep 用于回答“这个内容在哪里”。
        ...
        21. 探索顺序应尽可能遵循：
            确认边界 → 定向定位 → 读取目标 → 根据代码关系继续定向定位 → 完成任务。
        ...
        每次获得新的工具或子 Agent 结果后，必须重新判断当前任务是否已经满足完成条件。
        ...
        不要为了提高“信息完整度”而继续读取与当前结论没有直接关系的文件。
        
        特别是：
        如果已经获得能够支撑最终结论的最小证据集合，
        即使仍然存在其他“可能相关”的文件，也不要继续探索。
