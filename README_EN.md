...
EXPLORER: list_files / glob / grep / read_file / parse_java_code (Deprecated)

FIXER: list_files / glob / grep / read_file / write_file / apply_patch / delete_file / search_manual / verify_java_syntax (Deprecated) / run_command
...
Prepare MySQL (create database code_fix), Redis, RocketMQ.

Modify the connection configurations in CodeFix_Java/src/main/resources/application.yaml (MySQL address / credentials, Redis, mq.rocketmq.name-server, consumer / producer group and topic).
...
Copy the template and fill in environment variables (LLM, MySQL/Redis/RocketMQ, Ollama, Embedding, Workspace root, etc.):

bash
cp .env.example .env
Start (defaults to 0.0.0.0:8000; subscribes to RocketMQ messages on startup):
...
cd CodeFix_Web
npm install
Development environment backend address defaults to http://localhost:8080 (see .env.development):
